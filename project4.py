...
class Decoder(nn.Module):
    def __init__(self, z_dim=32, im_chan=1, hidden_dim=64):
        ...
        self.decoder = nn.Sequential(
            # First Transposed Conv Block: output_chan * 2 -> hidden_dim * 2
            nn.ConvTranspose2d(z_dim, hidden_dim * 4, kernel_size=3, stride=2, padding=0),  # Output: 8x8
            nn.BatchNorm2d(hidden_dim * 4),
            nn.ReLU(inplace=True),

            # Second Transposed Conv Block: hidden_dim * 2 -> hidden_dim
            nn.ConvTranspose2d(hidden_dim * 4, hidden_dim * 2, kernel_size=3, stride=2, padding=1, output_padding=1),  # Output: 16x16
            nn.BatchNorm2d(hidden_dim * 2),
            nn.ReLU(inplace=True),

            # Third Transposed Conv Block: hidden_dim -> im_chan
            nn.ConvTranspose2d(hidden_dim * 2, hidden_dim, kernel_size=3, stride=2, padding=1, output_padding=1),  # Output: 32x32
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(inplace=True),

            # Final Transposed Conv Block: hidden_dim -> im_chan
            nn.ConvTranspose2d(hidden_dim, im_chan, kernel_size=3, stride=2, padding=1, output_padding=2)  # Output: original size (e.g., 64x64)
        )
...
def plot_losses(val_loss, train_loss, date=None):
...
if __name__ == "__main__":
    ...
    try:
        ...
        vae, train_losses, val_losses = train_model(date, epochs=1, z_dim=z_dim)
    ...
    # plot the losses
    plot_losses(val_losses, train_losses, date)
